...
- **Agrupar por feature.** Cada feature es un spec (issue `full-stack`). Sus tickets `front` y `back` se publican como sub-issues de ese spec; donde los sub-issues no estén habilitados, `Part of #<spec>` al inicio del body. El nombre de `<feature>` de `.scratch/` es el mismo que se usa en el título del spec.
