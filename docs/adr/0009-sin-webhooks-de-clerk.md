...
ADR 0008 dejó pendiente la sincronización entre el Usuario de Clerk y el Usuario del back, y anticipó un webhook de `user.created`. No lo hacemos: cada request trae el JWT de Clerk, así que el back crea el Usuario la primera vez que lo ve, con el `clerkId` del token y el nombre y el email que Clerk devuelve. Nombre y email se refrescan en ese mismo camino cuando cambiaron en Clerk.
...
_Nota: el Empleado ya no se crea al primer request; dejó de ser una identidad de Clerk (ADR 0011)._
